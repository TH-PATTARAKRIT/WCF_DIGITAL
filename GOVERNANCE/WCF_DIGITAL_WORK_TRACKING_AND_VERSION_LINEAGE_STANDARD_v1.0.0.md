...
Where:
- xxxx = four-digit report code
- yyy = three-digit execution cycle
...
One cycle must contain exactly one Prompt, one Result and one Review. Files must not be overwritten. Corrections require the next sequence.

## 3. Version lineage

Every controlled document must identify:
- document_id
- document_version
- previous_version_id
- previous_commit_sha
...
The current version must never be inferred only from the file date. Each report must maintain `control/CURRENT.yml` and `control/version-lineage.yml`.
...
Evidence -> GitHub controlled record -> Jira -> Google Sheets
...
- EVIDENCE_REQUIRED
- PROMPT_DRAFT
- WAITING_FOR_BOSS_AUTHORIZATION
- AUTHORIZED_FOR_EXECUTION
- CLAUDE_RUNNING
- WAITING_FOR_CLAUDE_RESULT
- WAITING_FOR_REVIEW
- CORRECTION_REQUIRED
- WAITING_FOR_RETEST
...
Every status record must include Current State, Current Owner, Next Action, Due Date, Blocker, Required Evidence and Last Updated.

## 7. Automation safeguards

- Make must use an idempotency key: Report Code + Sequence + Source Commit SHA
- One report may have only one active cycle
- Default maximum cycle: 5
- Stop immediately for evidence gaps, scope change, restricted-data risk, business decision, merge, Jira closure or Production Ready declaration
